...
class AnalysisMethod(object):
    ...
    def set_method_attributes(self):
        ...
        config_query = '''
            ...
            WHERE andd.analysisMethodVersionsId = ? 
                AND andd.definerType IN ('data', 'detector', 'loadData', 'metaData') 
        ...
        try:
            ...
            self._full_config = meta_list + load_list + blnk_list    
    ...
    def get_load_data_config_orig(self, load_id, field_order):
        ## DEPRICATED
        """Returns a list of field with a shared detector if
    ...
    def save_panel_associations(self, panel_list, method_id):
    ...
    ### Below is the table/row construction functions used to create/populate the analysis steps
class AnalysisData(AnalysisMethod):
    ...
    def get_specimen_info(self, runId):
        ...
        spec_info = {
            ...
            'received_date': sample_details.get('specimenReceiveDate', ''),
    ...
    def apply_meta_data(self, runId):
        ...
        for field in self.meta_config:
            ...
            meta_field['value'] = getRunMetaData(self.switchboard, field['input_name'], runId)
            ...
            for field in self.load_config:
                ...
                if rs.next():
                    ...
                    while rs.next():
                        ...
                        run_field['value'] = self.format_result(run_field, result) #rs.getString('result')
    ...
    def format_result(self, field, result):
        """Used to format the result if datetime
        Args:
            field: field object
        ...
        Returns:
            formatted result
        """
        if field.get('data_type') == 'dateTime' and result:
            date = self.DateFormatter(self.switchboard, result)
            return date.shortFormatDate()
        else:
            return result 
...
class SaveAnalysisData(AnalysisData):
        ...
        for row in table_object:
            ...
            for field in row[1:]:
                ...
                if definer_type == 'loadData':
                    self.update_load_data(field, is_control)
    ...
    def save_field_data(self, data_run_id, field, is_control=False):
        ...
        query = '''
            INSERT INTO {}(
                ...
                interpretationEventId, eventId, stepName)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)
        '''.format(table_name, run_id_col)
    ...
    def update_load_data(self, field, is_control=False):
        ...
        if field.get('interpretation_change'):
            try:
                ...
                self.switchboard.log("---*** SQL Exception AT UPDATE_LOAD_DATA ***----")
                self.switchboard.log("ERROR MESSAGE: " + str(e.message))
                self.switchboard.formResults.put('PROCESSINGSUCCESSFUL', 'false')
                raise
                return
