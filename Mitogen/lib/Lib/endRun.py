        ...
        try:
            ...
            if isComplete(currentMethodState):
                ...
                if isComplete(reportDetailsState):
                    ...
                    # Add loop to clean up reports for specimen that have multiple reports for the same panel
                    errorLocation = '<<<<<< CALL: getReportIdList IN endRun >>>>>>'
                    reportIdList = getReportIdList(self.switchboard, specMethId)

                    for reportId in reportIdList:
                        if reportId != reportDetailsState[0]:
                            errorLocation = '<<<<<< CALL: updateReportStatus IN endRun for additional report Id >>>>>>'
                            updateReportStatus(self.switchboard, 'data ready', reportId, eventId)
...
def getReportId(switchboard, specMethId):
    """Returns the report id associated with the specimen method
        **NOTE: this query is complex due to the linking of subsamples to reportSpecimens
    Args:
        specMethId: specimen method id
    Returns:
        reportId
    """
    try:
        query = '''
            ...
            INNER JOIN requestSpecimens rs1 
                ON rs1.specimenId IN (SELECT specimenId FROM requestSpecimens WHERE requestId = rf.requestId) 
            INNER JOIN specimenMethods sm1
                ON sm1.requestSpecimensId = rs1.id
            AND sm1.panelCode IN (SELECT panelCode FROM reportDefinitionPanels WHERE reportDefinitionVersionId = rdp.reportDefinitionVersionId)
            LEFT JOIN reportSpecimens rspec 
                ON rspec.specimenMethodsId = sm.id OR sm1.id = rspec.specimenMethodsId
            LEFT JOIN reportDetails rd 
                ON rd.id = rspec.reportDetailsId
            WHERE sm.id = ? AND rd.reportId IS NOT NULL LIMIT 1
        '''
        stmt = switchboard.connection.prepareStatement(query)
        stmt.setInt(1, specMethId)
        ...
        switchboard.log('---*** SQL EXCEPTION ENDING PARENT WORKFLOW ***---')
        switchboard.formResults.put('PROCESSINGSUCCESSFUL', 'false')
    except:
        switchboard.log('---*** GENERAL EXCEPTION ENDING PARENT WORKFLOW ***---')
        switchboard.formResults.put('PROCESSINGSUCCESSFUL', 'false')


def getReportIdList(switchboard, specMethId):
    """Returns the list of other report id's associated with the specimen method
        **NOTE: this query is complex due to the linking of subsamples to reportSpecimens
    Args:
        specMethId: specimen method id
    Returns:
    ...
    try:
        query = '''
            ...
            LEFT JOIN reportDetails rd 
                ON rd.id = rspec.reportDetailsId
            WHERE sm.id = ? AND rd.reportId IS NOT NULL
        '''
        stmt = switchboard.connection.prepareStatement(query)
        stmt.setInt(1, specMethId)
        switchboard.log(str(stmt))
        ...
        rl = []
        while rs.next():
            rl.append(rs.getString('reportId'))
...
def getRepDetailsState(switchboard, specMethId):
    ...
    try:
        ...
        thisReport = ''
        statusQuery = '''
            SELECT IFNULL(sm1.status, 'In Process') AS 'status'
            FROM specimenMethods sm
            INNER JOIN reportDefinitionPanels rdp 
                ON rdp.panelCode = sm.panelCode
            INNER JOIN requestSpecimens rs 
            ...
            INNER JOIN specimenMethods sm1
                ...
                AND sm1.panelCode IN (SELECT panelCode FROM reportDefinitionPanels WHERE reportDefinitionVersionId = rdp.reportDefinitionVersionId)
            LEFT JOIN reportSpecimens rspec 
                ON rspec.specimenMethodsId = sm.id
            WHERE sm.id = ?
        '''

        statusQueryStmt = switchboard.connection.prepareStatement(statusQuery)
        statusQueryStmt.setInt(1, specMethId)
        switchboard.log(str(statusQueryStmt))
        statusRs = statusQueryStmt.executeQuery()
        ...
        curState.insert(0, getReportId(switchboard, specMethId))
        statusRs.close()
        statusQueryStmt.close()
