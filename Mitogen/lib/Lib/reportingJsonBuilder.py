...
class ReportingBuilder(ReportBuilder):
    ...
    def getOverallResult(self):
        ...
        try:
            oQuery = '''
                SELECT 
                    IFNULL(varcharReviewResult, '') AS 'varcharReviewResult',
            ...
            while oRs.next():
                res = oRs.getString('varcharReviewResult')
    ...
    def getOverallPanelResults(self):
        ...
        try:
            pQuery = '''
                SELECT
                    IFNULL(rrd.varcharReviewResult, '') AS 'varcharReviewResult',
            ...
            while pRs.next():
                ...
                res = pRs.getString('varcharReviewResult')
        ...
        try:
            rQuery = '''
                SELECT 
                    ...
                    addef.dataType,
                    CASE addef.dataType
                        WHEN 'varchar' THEN IFNULL(repRes.varcharResult, '')
                        WHEN 'dateTime' THEN IFNULL(repRes.dateTimeResult, '')
                        WHEN 'decimal' THEN
                            CASE addef.sigFig
                                WHEN 0 THEN IFNULL(ROUND(repRes.decimalResult, 0), '')
                                WHEN 1 THEN IFNULL(ROUND(repRes.decimalResult, 1), '')
                                WHEN 2 THEN IFNULL(ROUND(repRes.decimalResult, 2), '')
                                WHEN 3 THEN IFNULL(ROUND(repRes.decimalResult, 3), '')
                                WHEN 4 THEN IFNULL(ROUND(repRes.decimalResult, 4), '')
                                WHEN 5 THEN IFNULL(ROUND(repRes.decimalResult, 5), '')
                                WHEN 6 THEN IFNULL(ROUND(repRes.decimalResult, 6), '')
                            END
                    END AS 'originalResult',                                                          
                    CASE addef.dataType
                        WHEN 'varchar' THEN IFNULL(repRes.varcharReviewResult, '')
                        WHEN 'dateTime' THEN IFNULL(repRes.dateTimeReviewResult, '')
                        WHEN 'decimal' THEN
                        CASE addef.sigFig            
                            WHEN 0 THEN IFNULL(ROUND(repRes.decimalReviewResult, 0), '')
                            WHEN 1 THEN IFNULL(ROUND(repRes.decimalReviewResult, 1), '')
                            WHEN 2 THEN IFNULL(ROUND(repRes.decimalReviewResult, 2), '')
                            WHEN 3 THEN IFNULL(ROUND(repRes.decimalReviewResult, 3), '')
                            WHEN 4 THEN IFNULL(ROUND(repRes.decimalReviewResult, 4), '')
                            WHEN 5 THEN IFNULL(ROUND(repRes.decimalReviewResult, 5), '')
                            WHEN 6 THEN IFNULL(ROUND(repRes.decimalReviewResult, 6), '')
                        END
                   END AS 'result',
            ...
            while rRs.next():
                ...
                originalResult = rRs.getString('originalResult')
                ...
                if dataType != 'dateTime':
                    ...
                    if formatDate == True:
                        ...
                        formatObj = DateFormatter(self.switchboard, originalResult)
                        originalResult = formatObj.shortFormatDate()

                units = rRs.getString('reportableUnits')
                analysisDataId = rRs.getString('analysisDataId')
                resultCode = rRs.getString('resultCode')
                analysisDataDefinitionId = rRs.getString('analysisDataDefinitionId')
                ...
                resDict['originalResult'] = originalResult
    ...
    def makeReportReviewObj(self, formatDate=False):
        ...
        try:
            ...
            reportDict['report']['reportDataInterpretation'] = self.getReportDataInterpretation()
    ...
    def getReportDataInterpretation(self):
        interpQuery = '''
            SELECT 
				 addef.id AS "analysisDataDefinitionId",	
			...
			FROM reportDetails rd
            INNER JOIN reportResultData repRes  	ON repRes.reportDetailsId = rd.id
			INNER JOIN analysisData ad  			ON ad.id = repRes.analysisDataId
            INNER JOIN analysisDataRuns adr  		ON ad.analysisDataRunsId = adr.id
            INNER JOIN analysisDataDefinition addef ON addef.id = ad.analysisDataDefinitionId			
			WHERE rd.reportId = ? 
                   AND repRes.currentResult = 1                    
				   AND addef.definerType = 'data'	
				   AND addef.dataType <> 'image'					   
			GROUP BY addef.id, addef.VALUE
        '''

        try:
            interpStmt = self.switchboard.connection.prepareStatement(interpQuery)
            interpStmt.setString(1, self.reportId)
            ...
            self.switchboard.log("---*** SQLException AT REPORTINGJSONBUILDER GETREPORTDATAINTERPRETATION ***----")
            self.switchboard.log("ERROR MESSAGE: " + str(e.message))
            self.switchboard.formResults.put('PROCESSINGSUCCESSFUL', 'false')
            raise
            return
            ...
            self.switchboard.log("---*** KEY ERROR AT REPORTINGJSONBUILDER GETREPORTDATAINTERPRETATION ***----")
            self.switchboard.log("ERROR MESSAGE: " + str(e.message))
            self.switchboard.formResults.put('PROCESSINGSUCCESSFUL', 'false')
            raise
            return
    ...
    def getReviewImages(self):
        ...
        resQuery = '''
            SELECT
                ...
                IFNULL(rrd.interpretation, IFNULL(ad.actualInterpretation, '')) AS 'interpretation',
                IFNULL(rrd.wording, '') AS 'wording'
