...
class CreateRun(Run):
  ...
  def saveCreateRun(self):
    try:
      ...
      #### TODO: Check values types before running insert
      ...
      print 'runType in create run is ' + runType

      if(runType == 'process'):
        parentId = getProcessRunWorkflowParentRunId(self, runId)
      ...
      insertContent(self.switchboard, runId, 'run', 'runId', processContainerId, eventId)
      ...
      self.switchboard.log('<<<< NEW RUN FOR ' + processContainerId + ' CREATED: ' + runId )
...
def insertSpecimenRuns(switchboard, runId, specimenMethodsId, runType, currentContainerId, currentParentId, currentParentPosition, parentWorkflowSpecimenRunId, eventId, lastUpdatedEventId):
  try:
    ...
    print 'insertSpecimenRuns insertSpecimenRunsQueryStmt ' + str(insertSpecimenRunsQueryStmt)
