class AppTree(object):
    ...
    def buildTree(self, configVersion = None):
        """Builds node tree that consumed by engine for outbound messaging to DI
        Args:
            configVersion: ???
        Returns:
            appTree: uniflow node tree with sample/order/patient data
        """
        ...
        try:
            ...
            for key in nodeMap["runs"]:
                ...
                runObj = self.LabProcess(self.switchboard, runId, key, self.currentStep, self.containerType)
                run_node = runObj.buildRunNode(specimenIndex)
                self.switchboard.log('LAB PROCESS NODE FOR {}: {}'.format(runId, str(run_node)))
