...
class Run(object):
    ...
    def panel(self):
        query = '''
            SELECT panelCode FROM specimenMethods WHERE id = ?
        '''
        stmt = self.switchboard.connection.prepareStatement(query)
        stmt.setString(1, self.specimen_method)
        self.switchboard.log(str(stmt))
        rs = stmt.executeQuery()
        if rs.next():
            return rs.getString('panelCode')
        else:
            return ''
    ...
    def sampleDetails(self):
        ...
        s_query = '''
            SELECT
              IFNULL(rs.receivedDate, '') AS 'specimenReceivedDate',
              ...
              IFNULL(p.familyId, '') AS 'familyId',
              IFNULL(p.geneticGender, '') AS 'geneticGender',
        ...
        try:
            ...
            if rs.next():
                sample_details = {
                    'specimenReceivedDate': rs.getString('specimenReceivedDate'),
                    ...
                    'geneticGender': rs.getString('geneticGender'),                    
